...
- **Rodada no sandbox (16/09): PIX validado de ponta a ponta; cartão BLOQUEADO por homologação de conta, não testado.** O AbacatePay recusou o checkout hospedado com `CARD is not available for this store` — não é defeito de código, e não se "corrige" aqui. O nome do evento de conclusão do cartão segue sem confirmação. **Não trate a seção de cartão do roteiro como pendência de execução**: ela só pode ser feita depois da homologação.
