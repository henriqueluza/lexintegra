...
- **quem lê esse log.** A conta humana com `roles/owner`, que já lê o Secret Manager, e a **SA padrão do Compute**, com `roles/editor`: essa tem `logging.logEntries.list` e **não** tem `secretmanager.versions.access`, então o log ampliaria o acesso dela. Indiretamente, também quem emite token para essa SA (`api-lexintegra-run` e `firebase-adminsdk-fbsvc`, com `serviceAccountTokenCreator` no projeto inteiro). *(Bloco D: a concessão de projeto da `api-lexintegra-run` saiu, e ela só emite token para si mesma, que é o que a assinatura de URL exige — ADR-17. A da `firebase-adminsdk-fbsvc` é gerida pelo Firebase e segue no projeto, pendente de decisão.)*
...
**Sobre `checkouts` (Etapa 8).** É a coleção de vida mais curta do sistema, e a que mais carrega dado de quem ainda não é cliente: nome e e-mail do comprador e o carrinho. Dois carimbos, com papéis diferentes de propósito — `expiraEm` é até quando a cobrança pode ser paga; `apagarApos` é quando o documento some, igual ao vencimento da cobrança mais 48 horas, e **sempre escrito**. A **TTL nativa do Firestore** sobre `apagarApos` está declarada em `infra/terraform/firestore.tf`. O emulador não executa TTL: a prova é o `terraform plan` e um teste de unidade que garante o campo presente. A folga de 48 horas cobre o webhook tardio; depois dela, o pagamento vira `orfao` com alerta (ADR-19), e a evidência do aceite dos termos já foi copiada para o pagamento. A TTL cobre o carrinho abandonado **e também o pago**: a confirmação não mexe no prazo, e o que precisa ficar do checkout pago já foi copiado para o pagamento. A eliminação a pedido do titular é outro caminho: o Bloco A inventaria e exporta `checkouts` por `preCadastroId` (`apps/api/src/lgpd/mapa.ts`), e o executor destrutivo segue bloqueado até a política de guarda (seção 13).

**Conferido no Bloco B (23/09/2026): todo caminho que escreve em `checkouts` deixa `apagarApos` no documento.** `abrir` grava o documento inteiro com o prazo; `registrar` regrava o prazo pelo vencimento real da cobrança; `substituir`, `falhar` e a confirmação fazem `update` e o preservam. Um teste de integração passa pelos cinco e confere o campo em todos os documentos, e morde se um `update` o apagar. Nada do checkout fica fora dele com dado pessoal: os pagamentos `orfao`, `divergente` e `conflito_de_conta` guardam só ids e valor, o `confirmado` se liga ao cliente e não ao checkout, o checkout não gera evento de outbox, `produtos-gateway` não tem dado pessoal e o PIX sai sem `customer`. O que o comprador digita na página hospedada do cartão fica no AbacatePay, com a retenção dele.
...
**Índices compostos.** Declarados no Terraform — os das consultas da aplicação em `infra/terraform/firestore.tf`, os da sonda de sinais em `infra/terraform/sinais.tf` —, nunca criados à mão no console. O número não fica escrito aqui, porque envelhecia: `grep -c 'resource "google_firestore_index"' infra/terraform/*.tf` dá a contagem (14 em 25/09/2026, 4 deles de grupo de coleções) — o emulador não exige índice, então uma consulta sem índice declarado passa local e falha em produção. A regra é um índice por consulta que existe, não por consulta imaginável: índice composto custa escrita em toda gravação da coleção.
...
- **"Advogados com disponibilidade publicada e sem link de reunião" existe como métrica e política, sem emissor.** Não há campo de reunião no modelo — é Etapa 10. A política dispara com `> 0` e, sem dado, não dispara nunca; emitir `0` seria fingir medição. *(Nota de 25/09/2026, Bloco E: a Etapa 10 não chegou a emitir o sinal. Ele passou a ser emitido pela sonda de sinais, só com o Teams ligado, e ganhou um par: a idade da reunião em `reservada_sem_link`. Ver `apps/api/src/sinais/reunioes.ts` e o runbook `reuniao-sem-link.md`.)*
...
*Nota de 25/09/2026 (Bloco E, achado 4.9): `RASTREIO_HOOK_ESM` nunca existiu no código — nenhum commit em `apps/` a menciona; ela entrou só na documentação. A decisão acima vale como está: o gancho de ESM não é carregado. Mas não há variável que o ligue; ligá-lo seria mudar `apps/api/src/observabilidade/instrumentacao.ts`.*
...
**Exclusão automática: só `checkouts`.** É a única coleção com TTL — o documento some depois do vencimento da cobrança mais 48 horas (seção 5.1). Dois fatos que valem para a política, sem decidi-la: o Firestore apaga pela TTL normalmente em até 24 horas depois do prazo, e o **PITR está ligado** (`infra/terraform/firestore.tf`), então a versão apagada continua recuperável por até 7 dias. O restante — cadastro, pré-cadastro, anamnese, pedidos — não tem prazo automático, e isso é a tensão acima.
